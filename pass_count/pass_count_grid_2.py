...
click_pt_1 = [53.312079, -113.577580 ]
click_pt_2 = [53.310733, -113.576996 ]
...
grid = np.empty( [ len(lons), len(lats), 4]  )
...
print('grid size : ' + str( grid.shape) )
print('layer content: ')
print(str(road_area_layer) + ' = road_area_layer')
print(str(pass_count_layer) + ' = pass_count_layer')
print(str(lat_int_layer) + ' = lat_int_layer')
print(str(lon_int_layer) + ' = lon_int_layer')


for i in range(len(lats)):
    aaaaaaa = lats[i]
    grid[ : , i ,  lat_int_layer] = aaaaaaa
    
for i in range(len(lons)):
    aaaaaaa = lons[i]
    grid[ i , : ,  lon_int_layer] = aaaaaaa
...
def get_point_grid_loc( pt ):
    ...
    lat_index = math.floor( (pt_lat - lats[0]) / cell_size )
    lon_index = math.floor( (pt_lon - lons[0]) / cell_size )
