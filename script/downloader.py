...
class MapDownloader:
    def __init__(self, tile_file="global_tile_coord.json"):
        if not os.path.exists(tile_file):
            ...
            with open(tile_file) as f:
                self.global_tile_content = json.load(f)
    ...
    def download_callback(self, data):
        ...
        #print('callback')
        self.update_download_list(x_index, y_index)
    ...
    def update_download_list(self, x_index, y_index):
        download_list = set()
        x_check_list = range(x_index-self.cache_size, x_index+self.cache_size+1)
...
rospy.init_node('dlnode', anonymous=True)
m = MapDownloader()
m.main()
